...
class PhaseA {
public:
    PhaseA() : _pzem(Serial1, PZEM1_RX, PZEM1_TX) {}
    ...
    PhaseData read() {
        ...
        if (_failCount >= MAX_FAILS && (int32_t)(millis() - _nextRetry) < 0) {
            ...
            d.energy = _last.energy; // preserve last valid cumulative energy
            _last = d;
            return d;
        }
    ...
    float readFrequency() {
        if (_failCount >= MAX_FAILS) return _lastFreq;
        float f = _pzem.frequency();
        if (!isnan(f)) _lastFreq = f;
        return _lastFreq;
    }
    ...
    float getLastFreq() const { return _lastFreq; }
    bool isConnected() const { return _failCount < MAX_FAILS; }

private:
    PZEM004Tv30 _pzem;
    PhaseData _last;
    float _lastFreq = 50.0;
    int _failCount = 0;
