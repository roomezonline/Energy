...
public class ReportsController : ControllerBase
{
    ...
    public ReportsController(AppDbContext db, ILogger<ReportsController> log)
    {
        _db = db;
        _log = log;
    }
    ...
    public async Task<IActionResult> GetDaily(Guid centerId, [FromQuery] string? from, [FromQuery] string? to)
    {
        ...
        var fromDate = string.IsNullOrEmpty(from)
            ? new DateTime(iranNow.Year, iranNow.Month, 1)
            : ParsePersianDate(from, pc);
        var toDate = string.IsNullOrEmpty(to)
            ? iranNow
            : ParsePersianDate(to, pc).AddDays(1);

        var fromUtc = TimeZoneInfo.ConvertTimeToUtc(fromDate, iranTz);
        var toUtc = TimeZoneInfo.ConvertTimeToUtc(toDate, iranTz);

        var cons = await _db.EnergyConsumptions
            .Where(s => s.DeviceId == device.DeviceId && s.Timestamp >= fromUtc && s.Timestamp < toUtc)
            .OrderBy(s => s.Timestamp)
            .ToListAsync();

        var daily = new List<object>();
        var grouped = cons.GroupBy(s => TimeZoneInfo.ConvertTimeFromUtc(s.Timestamp, iranTz).Date);

        foreach (var day in grouped.OrderBy(g => g.Key))
        {
            var list = day.ToList();
            var dA = list.Sum(s => s.DeltaA);
            var dB = list.Sum(s => s.DeltaB);
            var dC = list.Sum(s => s.DeltaC);

            var pDate = $"{pc.GetYear(day.Key):D4}/{pc.GetMonth(day.Key):D2}/{pc.GetDayOfMonth(day.Key):D2}";
            daily.Add(new
            {
                date = pDate,
                phaseAKWh = Math.Round(dA, 4),
                phaseBKWh = Math.Round(dB, 4),
                phaseCKWh = Math.Round(dC, 4),
                totalKWh = Math.Round(dA + dB + dC, 4)
            });
        }
    ...
    public async Task<IActionResult> GetMonthly(Guid centerId, [FromQuery] int year, [FromQuery] int? compareTo)
    {
        ...
        for (int m = 1; m <= 12; m++)
        {
            ...
            var end = m < 12
                ? TimeZoneInfo.ConvertTimeToUtc(pc.ToDateTime(year, m + 1, 1, 0, 0, 0, 0), iranTz)
                : TimeZoneInfo.ConvertTimeToUtc(pc.ToDateTime(year + 1, 1, 1, 0, 0, 0, 0), iranTz);

            var total = await _db.EnergyConsumptions
                .Where(s => s.DeviceId == device.DeviceId && s.Timestamp >= start && s.Timestamp < end)
                .SumAsync(s => s.DeltaA + s.DeltaB + s.DeltaC);
            ...
            if (compareTo.HasValue)
            {
                ...
                var cEnd = m < 12
                    ? TimeZoneInfo.ConvertTimeToUtc(pc.ToDateTime(compareTo.Value, m + 1, 1, 0, 0, 0, 0), iranTz)
                ...
                compareTotal = await _db.EnergyConsumptions
                    .Where(s => s.DeviceId == device.DeviceId && s.Timestamp >= cStart && s.Timestamp < cEnd)
                    .SumAsync(s => s.DeltaA + s.DeltaB + s.DeltaC);
            }
            ...
            result.Add(new
            {
                month = m,
                year,
                totalKWh = Math.Round(total, 4),
                compareToYear = compareTo,
                compareToKWh = compareTotal.HasValue ? (decimal?)Math.Round(compareTotal.Value, 4) : null
            });
        }
    ...
    public async Task<IActionResult> GetPeak(Guid centerId, [FromQuery] string? from, [FromQuery] string? to)
    {
        ...
        var peak = await _db.EnergyConsumptions
            .Where(s => s.DeviceId == device.DeviceId && s.Timestamp >= fromUtc && s.Timestamp < toUtc)
            .OrderByDescending(s => s.DeltaA + s.DeltaB + s.DeltaC)
            .Select(s => new { s.Timestamp, total = s.DeltaA + s.DeltaB + s.DeltaC })
            .FirstOrDefaultAsync();
        ...
        if (peak != null)
        {
            ...
            return Ok(new
            {
                peakKWh = Math.Round(peak.total, 4),
                peakDate = pDate,
                peakHour = iranPt.Hour
            });
        }
        ...
        [FromQuery] string from2, [FromQuery] string to2)
    {
        ...
        decimal PeriodTotal(string from, string to)
        {
            var f = ParsePersianDate(from, pc);
            var t = ParsePersianDate(to, pc).AddDays(1);
            ...
            return _db.EnergyConsumptions
                .Where(s => s.DeviceId == device.DeviceId && s.Timestamp >= fUtc && s.Timestamp < tUtc)
                .Sum(s => s.DeltaA + s.DeltaB + s.DeltaC);
        }
